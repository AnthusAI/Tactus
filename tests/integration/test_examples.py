...
@pytest.mark.asyncio
async def test_example_executes(example_file: Path, load_example):
    """
    ...
    if config.get("outputs"):
        OutputModel = create_output_model_from_schema(
            config["outputs"], f"{config['name'].replace('_', '').title()}Output"
        )
    ...
    ), f"Example {example_file.name} failed to execute: {result.get('error', 'Unknown error')}"
...
async def test_hello_world_outputs_correct_values(
    example_runtime: TactusRuntime, examples_dir: Path, load_example
):
    ...
    config = load_example(example_file)
    OutputModel = create_output_model_from_schema(config["outputs"], "HelloWorldOutput")
...
async def test_state_management_outputs_correct_count(
    ...
    config = load_example(example_file)
    OutputModel = create_output_model_from_schema(config["outputs"], "StateManagementOutput")
...
async def test_with_parameters_uses_defaults(
    ...
    config = load_example(example_file)
    OutputModel = create_output_model_from_schema(config["outputs"], "WithParametersOutput")
...
async def test_with_parameters_accepts_overrides(
    ...
    context = {"task": "custom task", "count": 7}
...
@pytest.mark.asyncio
async def test_simple_agent_calls_llm_and_done_tool(examples_dir: Path, load_example):
    """
    ...
    assert (
        "not complete" not in greeting.lower()
    ), "Greeting indicates agent did not complete properly"
