        ...
        dsl_keywords = [
            {
                "label": "Agent",
                "kind": 3,  # Function
                "insertText": 'Agent {\n\tmodel = "${1:openai/gpt-4o-mini}",\n\tsystem_prompt = "${2:You are helpful}",\n\ttools = {${3:}}\n}',
    ...
    def get_hover(self, uri: str, position: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if hasattr(registry, "parameters") and registry.parameters:
            ...
            for param_name, param_decl in registry.parameters.items():
                ...
                type_str = (
                    param_type.value if param_type and hasattr(param_type, "value") else "unknown"
                )
        ...
        if hasattr(registry, "outputs") and registry.outputs:
            ...
            for output_name, output_decl in registry.outputs.items():
                ...
                type_str = (
                    field_type.value if field_type and hasattr(field_type, "value") else "unknown"
                )
        ...
        signatures = [
            {
                ...
                "parameters": [
                    {
                        "label": "provider",
                        "documentation": "LLM provider (openai, bedrock, google)",
                    },
                    {
                        "label": "model",
                        "documentation": "Model name (gpt-4o, claude-opus-4-5, etc.)",
                    },
