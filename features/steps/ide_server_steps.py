...
def step_backend_on_port(context, port):
    ...
    assert f"Backend port: {port}" in context.ide_output, \
...
def step_frontend_on_port(context, port):
    ...
    assert f"Frontend port: {port}" in context.ide_output, \
...
def step_browser_opens(context, url):
    ...
    assert f"Opening browser to {url}" in context.ide_output or \
           "Opening browser" in context.ide_output
...
def step_see_port_conflict_note(context, port):
    ...
    assert f"Port {port} in use" in context.ide_output or \
           f"using" in context.ide_output.lower()
...
def step_browser_to_detected_port(context):
    ...
    assert hasattr(context, "detected_frontend_port"), \
        "Frontend port was not detected"
...
def step_frontend_connects_to_backend(context):
    ...
    assert hasattr(context, "detected_backend_port"), \
        "Backend port was not detected"
...
def step_terminal_1_shows(context, text):
    ...
    assert text in context.ide_output_1, \
...
def step_terminal_2_different_port(context, port_type):
    ...
    assert port1 != port2, \
        f"Both terminals using same {port_type} port: {port1}"
...
def step_see_frontend_url(context):
    ...
    assert "http://localhost:" in context.ide_output or \
           "Frontend port:" in context.ide_output
...
def step_see_error(context):
    ...
    assert context.ide_process.returncode != 0 or \
           "error" in context.ide_output.lower()
