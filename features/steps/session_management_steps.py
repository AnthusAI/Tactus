...
def step_impl(context):
    _store(context).record_message(_active_session(context), "system", "Approval requested", "human_interaction")


@when('I record the human response')
def step_impl(context):
    _store(context).record_message(_active_session(context), "human", "Approved", "human_interaction")
...
def step_impl(context):
    for idx in range(3):
        session_id = _store(context).start_session({"task_type": "research" if idx % 2 == 0 else "ops"})
