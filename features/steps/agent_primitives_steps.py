...
class MockAgentPrimitive:
    ...
    def call(self, prompt, **kwargs):
        ...
        calc_indicators = ["calculate", "%", "percent", "of", "multiply", "divide", "add", "subtract"]
...
def step_impl(context, message):
    ...
    context.llm.set_response(MockAIMessage(f"Python is a programming language"))
...
def step_impl(context, message):
    ...
    context.llm.set_response(MockAIMessage(f"Type hints were introduced in Python 3.5"))
    context.agent_response = context.agent.call(message)
    context.conversation.append((message, context.agent_response['content']))
...
def step_impl(context):
    ...
    assert has_error or invalid_response, \
