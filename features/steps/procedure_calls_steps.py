...
def step_impl(context, name):
    params = {row["param"]: int(row["value"]) if row["value"].isdigit() else row["value"] for row in context.table}
    result, state = _runtime(context).call(name, params=params, state=_proc_state(context)["main_state"])
...
def step_impl(context):
    result, state = _runtime(context).call("mutating_subproc", state=_proc_state(context)["main_state"].copy())
    _proc_state(context)["last_result"] = result
    _proc_state(context)["last_child_state"] = state
...
def step_impl(context):
    assert _proc_state(context)["last_child_state"]["received"] == _proc_state(context)["last_result"]
...
def step_impl(context):
    ...
    _runtime(context).register_callable("level3", lambda params, s, r: state["call_history"].append("level3"))
