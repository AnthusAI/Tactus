...
def step_impl(context, checkpoint_name):
    ...
    assert position == 0 or checkpoint_name not in context.checkpoint_positions, f"Checkpoint {checkpoint_name} should not exist yet"
...
def step_impl(context, checkpoint_name):
    ...
    if checkpoint_name in checkpoint_positions:
        ...
        assert position < len(metadata.execution_log), f"Checkpoint {checkpoint_name} at position {position} should exist"
    else:
...
def step_impl(context, checkpoint_name, expected):
    ...
    position = checkpoint_positions[checkpoint_name]
    assert position < len(metadata.execution_log), f"Checkpoint at position {position} does not exist"
