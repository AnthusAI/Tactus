...
def step_impl(context):
    ...
    assert context.execution_order == expected_order, \
...
def step_impl(context):
    ...
    context.workflow_config = {
        ...
        'error_handlers': {
            'step2': {'action': 'log', 'params': {'message': 'Step 2 failed'}}
        }
    }
        ...
        'steps': [
            {
                ...
                'then': [
                    {'id': 'prod_step', 'action': 'state.set', 'params': {'key': 'path', 'value': 'production'}}
                ],
                'else': [
                    {'id': 'dev_step', 'action': 'state.set', 'params': {'key': 'path', 'value': 'development'}}
                ]
            }
        ]
    }
...
def step_impl(context, seconds):
    ...
    context.workflow_config = {
        ...
        'timeout': seconds,
        'steps': [
            {'id': 'long_step', 'action': 'sleep', 'params': {'duration': seconds + 10}}
        ]
    }
...
def step_impl(context):
    ...
    for i, step in enumerate(steps, 1):
        ...
        if context.execution_context.storage.checkpoint_exists(context.procedure_id, checkpoint_name):
...
def step_impl(context, step_list):
    ...
    for step_num in steps:
        ...
        assert f"skipped_{step_id}" in context.resumed_execution, \
...
def step_impl(context, step_num):
    ...
    step_id = f"step{step_num}"
    assert f"executed_{step_id}" in context.resumed_execution, \
